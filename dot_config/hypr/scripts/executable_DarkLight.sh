...
# Force dark mode and static wallpaper
if true; then
    echo "Dark" > "$HOME/.cache/.theme_mode"
    gsettings set org.gnome.desktop.interface color-scheme 'prefer-dark' > /dev/null 2>&1
    gsettings set org.gnome.desktop.interface gtk-theme "Flat-Remix-GTK-Blue-Dark" > /dev/null 2>&1
    gsettings set org.gnome.desktop.interface icon-theme "Flat-Remix-Blue-Dark" > /dev/null 2>&1
    $swww "$HOME/Firefox_wallpaper.png" $effect
    ${SCRIPTSDIR}/WallustSwww.sh "$HOME/Firefox_wallpaper.png" > /dev/null 2>&1
    notify-send -u low -i "$notif" "Theme locked" "Dark mode + static wallpaper"
    exit 0
fi
